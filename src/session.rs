...
impl Session {
    ...
    /// 座標 `(row, col)` を返す。
    ///
    /// [`Session::hint`] / [`Session::deduce`] と違い現盤面を推論の種に
    /// しないため、ユーザーの誤記入が判定の前提に混ざらない。比較は
    /// 「ユーザー側が未記入でない」かつ「解答側が確定している」セルに
    /// ついてのみ行う。制約だけでは一意に解けない場合もエラーにはせず、
    /// 確定できたセルに限って検出する。制約自体が矛盾していて解を
    /// 持たない場合は `Err` を返す。
