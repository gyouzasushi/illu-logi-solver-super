...
//!
//! パズル入力はまず [`Clues::new`] で検証済みの [`Clues`] に「パース」され、
//! 以降のAPIはすべて検証済みの値だけを受け取る（parse, don't validate）。
//! これにより [`crate::Solver::new`] や [`crate::Session::new`] は失敗しない。
...
impl LineId {
    ...
    /// この行・列と `offset` 番目のセルで交差する、直交する行・列。
    ///
    /// ```
    /// use illu_logi_solver_super::LineId;
    ///
...
/// 検証済みのパズル制約。
///
/// 各行・各列の「黒マスの連続長の並び」を持つ。高さ（行の本数）と
/// 幅（列の本数）は独立で、長方形の盤面をそのまま扱える。
/// 構築は [`Clues::new`] のみで、構築できた時点で
/// 「ブロックサイズはすべて1以上」「どの行・列の制約も、間に白1マスずつを
/// 挟めば線長に収まる」ことが保証されている。
